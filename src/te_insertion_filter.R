...
for(i in 1:nrow(idf)){
    ...
    # Parental to be zero supporting read
    if(!((tmp[V15==fs, V10] == 0)  & (tmp[V15==ms, V10]==0))) next
    # Allelic balance
    ab = max(tmp$V13)
    if(tmp[V13==ab,.N]>1) next # multiple max freqs
    if(!ab %between% c(MIN_FREQ, 1 - MIN_FREQ)) next
    # all non-focal samples should meet NON_FOCAL_MAX_READ
    if(!(tmp[V10 <= NON_FOCAL_MAX_READ, .N] == (sample_N -1))) next
    # number of impurity samples
    if(!(tmp[V10!=0,.N] <= (IMPURITY_SAMPLE_MAX + 1))) next
...
if(ay[,.N]!=0){
    ...
    for(i in 1:nrow(idf)){
        ...
        # all males should meet minimum read depth/2 on Y
        if(!all(tmp[V15 %in% male_samples,V10+V11] >= MIN_TOTAL_READ/2)) next

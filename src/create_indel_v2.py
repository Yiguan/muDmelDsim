...
parser.add_argument('-simufile', help='simulation file that contains the random sites')
...
def refCount(depth):
    ...
    with open(ALT_FREQ, 'r') as ff:
        ...
        for ll in lines:
            lv = ll.strip().split('\t')
            if lv[0]==str(depth) and len(lv)==2:
                ref_pool = lv[1].split(',')
                ref_count = random.sample(ref_pool,1)[0]
                return ref_count
        print(f'Use binomial distribution for sampling at Depth:{depth}')
        ref_count = np.random.binomial(depth, 0.5, 1)
        return ref_count
...
def main():
    sams = [x for x in os.listdir('.') if re.match(r".[0-9]*_[0-9a-zA-Z]*_[0-9]*.sam$", x)]
